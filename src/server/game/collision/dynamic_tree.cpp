...
#include "bounding_interval_hierarchy_wrapper.h"
...
struct DynamicTreeIntersectionCallback
{
    ...
    {
        did_hit = obj.intersectRay(r, distance, true, phase_mask, ModelIgnoreFlags::Nothing);
        return did_hit;
    }
...
struct DynamicTreeIntersectionCallback_WithLogger
{
    ...
    bool operator()(G3D::Ray const& r, GameObjectModel const& obj, float& distance)
    {
        bool hit = obj.intersectRay(r, distance, true, phase_mask, ModelIgnoreFlags::Nothing);
        if (hit)
        {
            did_hit = true;
        }
