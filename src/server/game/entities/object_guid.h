...
////GUID_TRAIT_MAP_SPECIFIC(HighGuid::Entity)
...
////	bool IsGameObject()        const { return GetHigh() == HighGuid::Entity; }
...
////		case HighGuid::Entity:   return TYPEID_GAMEOBJECT;
...
////	virtual ObjectGuid::LowType Generate(HighGuid high) = 0;
...
////
////    ObjectGuid::LowType Generate(HighGuid high) override
////    {
////        if (_nextGuid >= ObjectGuid::GetMaxCounter(high) - 1)
////            HandleCounterOverflow(high);
////
////        if (high == HighGuid::Unit || high == HighGuid::Entity)
...
//	uint64   GetRawValue() const { return _guid; }
//	HighGuid GetHigh() const { return HighGuid((_guid >> 48) & 0x0000FFFF); }
//	uint32   GetEntry() const { return HasEntry() ? uint32((_guid >> 24) & 0x0000000000FFFFFFull) : 0; }
//	LowType  GetCounter()  const
//	{
//		return HasEntry()
//			? LowType(_guid & 0x0000000000FFFFFFull)
//			: LowType(_guid & 0x00000000FFFFFFFFull);
//	}
//
//	static LowType GetMaxCounter(HighGuid high)
//	{
...
//	bool operator< (ObjectGuid const& guid) const { return GetRawValue() < guid.GetRawValue(); }
//
//	static std::string GetTypeName(HighGuid high)
//	{
//		switch (high)
//		{
//		case HighGuid::Item:         return "Item";
//		case HighGuid::Player:       return "Player";
//		case HighGuid::Entity:   return "Gameobject";
...
//	}
//
//	std::string GetTypeName() const { return !IsEmpty() ? GetTypeName(GetHigh()) : "None"; }
//	std::string ToString() const
//	{
...
//	bool HasEntry() const { return HasEntry(GetHigh()); }
//
//	static ObjectGuid Global(HighGuid type, LowType counter) { return ObjectGuid(type, counter); }
//	static ObjectGuid MapSpecific(HighGuid type, uint32 entry, LowType counter) { return ObjectGuid(type, entry, counter); }
//
//	explicit ObjectGuid(uint32 const&) = delete;                 // no implementation, used to catch wrong type assignment
//	ObjectGuid(HighGuid, uint32, uint64 counter) = delete;       // no implementation, used to catch wrong type assignment
//	ObjectGuid(HighGuid, uint64 counter) = delete;               // no implementation, used to catch wrong type assignment
...
//	static void HandleCounterOverflow(HighGuid high)
//	{
//		LOG_ERROR << fmt::format("{} guid overflow!! Can't continue, shutting down server. ", ObjectGuid::GetTypeName(high));
//	}
//	static void CheckGuidTrigger(ObjectGuid::LowType guid) {};
//	ObjectGuid::LowType _nextGuid;
//};
