...
//#include "../entities/creature/gossip_def.h"
//#include "../grids/grid_notifiers.h"
...
//	if (!source && !target)
//	{
//		LOG_ERROR << fmt::format("scripts", "%s source and target objects are NULL.", scriptInfo->GetDebugInfo().c_str());
//	}
//    else
//    {
...
//
//        if (!player)
//             LOG_ERROR << fmt::format("%s neither source nor target object is player (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
...
//    if (!source && !target)
//         LOG_ERROR << fmt::format("%s source and target objects are NULL.", scriptInfo->GetDebugInfo().c_str());
//    else
//    {
//        if (bReverse)
//        {
...
//
//        if (!creature)
//             LOG_ERROR << fmt::format("%s neither source nor target are creatures (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
//                scriptInfo->GetDebugInfo().c_str(),
//                source ? source->GetTypeId() : 0, source ? source->GetEntry() : 0, source ? source->GetGUID().GetCounter() : 0,
//                target ? target->GetTypeId() : 0, target ? target->GetEntry() : 0, target ? target->GetGUID().GetCounter() : 0);
//    }
...
//    Unit* unit = nullptr;
//    if (!obj)
//         LOG_ERROR << fmt::format("%s %s object is NULL.", scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
//    else if (!obj->isType(TYPEMASK_UNIT))
//         LOG_ERROR << fmt::format("%s %s object is not unit (TypeId: %u, Entry: %u, GUID: %u), skipping.",
//            scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(), obj->GetGUID().GetCounter());
//    else
//    {
//        unit = obj->ToUnit();
//        if (!unit)
//             LOG_ERROR << fmt::format("%s %s object could not be cast to unit.",
...
//        player = obj->ToPlayer();
//        if (!player)
//             LOG_ERROR << fmt::format("%s %s object is not a player (TypeId: %u, Entry: %u, GUID: %u).",
//                scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(), obj->GetGUID().GetCounter());
//    }
//    return player;
//}
...
//        if (!creature)
//             LOG_ERROR << fmt::format("%s %s object is not a creature (TypeId: %u, Entry: %u, GUID: %u).", scriptInfo->GetDebugInfo().c_str(),
...
//    if (!obj)
//         LOG_ERROR << fmt::format("%s %s object is NULL.",
//            scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target");
//    else
//    {
//        pWorldObject = dynamic_cast<WorldObject*>(obj);
//        if (!pWorldObject)
//             LOG_ERROR << fmt::format("%s %s object is not a world object (TypeId: %u, Entry: %u, GUID: %u).",
//                scriptInfo->GetDebugInfo().c_str(), isSource ? "source" : "target", obj->GetTypeId(), obj->GetEntry(), obj->GetGUID().GetCounter());
//    }
//    return pWorldObject;
//}
...
//        default:
//             LOG_ERROR << fmt::format("%s unknown command for _ScriptProcessDoor.", scriptInfo->GetDebugInfo().c_str());
//            return;
//    }
//    if (!guid)
//         LOG_ERROR << fmt::format("%s door guid is not specified.", scriptInfo->GetDebugInfo().c_str());
//    else if (!source)
//         LOG_ERROR << fmt::format("%s source object is NULL.", scriptInfo->GetDebugInfo().c_str());
//    else if (!source->isType(TYPEMASK_UNIT))
//         LOG_ERROR << fmt::format("%s source object is not unit (TypeId: %u, Entry: %u, GUID: %u), skipping.", scriptInfo->GetDebugInfo().c_str(),
//            source->GetTypeId(), source->GetEntry(), source->GetGUID().GetCounter());
//    else
//    {
//        WorldObject* wSource = dynamic_cast <WorldObject*> (source);
//        if (!wSource)
//             LOG_ERROR << fmt::format("%s source object could not be cast to world object (TypeId: %u, Entry: %u, GUID: %u), skipping.",
...
//            if (!pDoor)
//                 LOG_ERROR << fmt::format("%s gameobject was not found (guid: %u).", scriptInfo->GetDebugInfo().c_str(), guid);
//            else if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR)
//                 LOG_ERROR << fmt::format("%s gameobject is not a door (GoType: %u, Entry: %u, GUID: %u).",
...
//                    break;
//                default:
//                     LOG_ERROR << fmt::format("%s source with unsupported high guid %s.",
...
//                    break;
//                default:
//                     LOG_ERROR << fmt::format("%s target with unsupported high guid %s.",
...
//                {
//                     LOG_ERROR << fmt::format("%s invalid chat type (%u) specified, skipping.", step.script->GetDebugInfo().c_str(), step.script->Talk.ChatType);
//                    break;
//                }
//
...
//                    if (!sourceUnit)
//                    {
//                         LOG_ERROR << fmt::format("%s source object (%s) is not an unit, skipping.", step.script->GetDebugInfo().c_str(), source->GetGUID().ToString().c_str());
//                        break;
//                    }
//
...
//                            if (!receiver)
//                                 LOG_ERROR << fmt::format("%s attempt to whisper to non-player unit, skipping.", step.script->GetDebugInfo().c_str());
//                            else
...
//                         LOG_ERROR << fmt::format("%s wrong field %u (max count: %u) in object (TypeId: %u, %s) specified, skipping.",
...
//                         LOG_ERROR << fmt::format("%s wrong field %u (max count: %u) in object (TypeId: %u, Entry: %u, GUID: %u) specified, skipping.",
...
//                if (!source)
//                {
//                     LOG_ERROR << fmt::format("%s source object is NULL.", step.script->GetDebugInfo().c_str());
//                    break;
//                }
//                if (!target)
//                {
//                     LOG_ERROR << fmt::format("%s target object is NULL.", step.script->GetDebugInfo().c_str());
//                    break;
//                }
//
...
//                    {
//                         LOG_ERROR << fmt::format("%s source is not unit, gameobject or player (TypeId: %u, Entry: %u, GUID: %u), skipping.",
...
//                        {
//                             LOG_ERROR << fmt::format("%s target is not unit, gameobject or player (TypeId: %u, Entry: %u, GUID: %u), skipping.",
...
//                    else
//                    {
//                         LOG_ERROR << fmt::format("%s neither source nor target is player (source: TypeId: %u, Entry: %u, GUID: %u; target: TypeId: %u, Entry: %u, GUID: %u), skipping.",
//                            step.script->GetDebugInfo().c_str(), source->GetTypeId(), source->GetEntry(), source->GetGUID().GetCounter(),
...
//                {
//                     LOG_ERROR << fmt::format("%s gameobject guid (datalong) is not specified.", step.script->GetDebugInfo().c_str());
//                    break;
//                }
//
...
//                    if (!pGO)
//                    {
//                         LOG_ERROR << fmt::format("%s gameobject was not found (guid: %u).", step.script->GetDebugInfo().c_str(), step.script->RespawnGameobject.GOGuid);
//                        break;
//                    }
//
...
//                    {
//                         LOG_ERROR << fmt::format("%s can not be used with gameobject of type %u (guid: %u).",
...
//                         LOG_ERROR << fmt::format("%s creature entry (datalong) is not specified.", step.script->GetDebugInfo().c_str());
//                    else
//                    {
...
//                             LOG_ERROR << fmt::format("%s creature was not spawned (entry: %u).", step.script->GetDebugInfo().c_str(), step.script->TempSummonCreature.CreatureEntry);
//                    }
//                }
//                break;
//            }
...
//                    {
//                         LOG_ERROR << fmt::format("%s target object is not gameobject (TypeId: %u, Entry: %u, GUID: %u), skipping.",
//                            step.script->GetDebugInfo().c_str(), target->GetTypeId(), target->GetEntry(), target->GetGUID().GetCounter());
//                        break;
//                    }
//
...
//                if (!source && !target)
//                {
//                     LOG_ERROR << fmt::format("%s source and target objects are NULL.", step.script->GetDebugInfo().c_str());
//                    break;
//                }
//
...
//                {
//                     LOG_ERROR << fmt::format("%s no source unit found for spell %u", step.script->GetDebugInfo().c_str(), step.script->CastSpell.SpellID);
//                    break;
//                }
//
//                if (!uTarget || !uTarget->isType(TYPEMASK_UNIT))
//                {
//                     LOG_ERROR << fmt::format("%s no target unit found for spell %u", step.script->GetDebugInfo().c_str(), step.script->CastSpell.SpellID);
//                    break;
//                }
//
...
//                         LOG_ERROR << fmt::format("%s source object has an invalid path (%u), skipping.", step.script->GetDebugInfo().c_str(), step.script->LoadPath.PathID);
//                    else
...
//                {
//                     LOG_ERROR << fmt::format("%s creature entry is not specified, skipping.", step.script->GetDebugInfo().c_str());
//                    break;
//                }
//                if (!step.script->CallScript.ScriptID)
//                {
//                     LOG_ERROR << fmt::format("%s script id is not specified, skipping.", step.script->GetDebugInfo().c_str());
//                    break;
//                }
//
...
//                if (!cTarget)
//                {
//                     LOG_ERROR << fmt::format("%s target was not found (entry: %u)", step.script->GetDebugInfo().c_str(), step.script->CallScript.CreatureEntry);
//                    break;
//                }
//
...
//                if (!datamap)
//                {
//                     LOG_ERROR << fmt::format("%s unknown scriptmap (%u) specified, skipping.", step.script->GetDebugInfo().c_str(), step.script->CallScript.ScriptType);
//                    break;
//                }
//
...
//                {
//                    if (cSource->isDead())
//                         LOG_ERROR << fmt::format("%s creature is already dead (Entry: %u, GUID: %u)",
...
//
//            default:
//                 LOG_ERROR << fmt::format("Unknown script command %s.", step.script->GetDebugInfo().c_str());
//                break;
//        }
//
