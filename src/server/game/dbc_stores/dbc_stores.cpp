...
#include "dbc_fileloader.h"
#include "dbc_fmt.h"
#include "../common/common_defines.h"
#include "../entities/object_defines.h"
                    ...
                    char const* dbTable = nullptr, char const* dbFormat = nullptr, char const* dbIndexName = nullptr)
{
	...
	std::cout << "load: " << dbcFilename << std::endl;
...
{
	std::cout << "load dbc, path: " << dataPath << std::endl;
...
#define LOAD_DBC(store, file) LoadDBC(availableDbcLocales, bad_dbc_files, store, dataPath, file)
...
#define LOAD_DBC_EXT(store, file, dbtable, dbformat, dbpk) LoadDBC(availableDbcLocales, bad_dbc_files, store, dataPath, file, dbtable, dbformat, dbpk)
...
#undef LOAD_DBC_EXT
        ...
        for (uint8 x = 0; x < MAX_DIFFICULTY; ++x)
        {
            if (spellDiff->SpellID[x] <= 0 || !sSpellStore.LookupEntry(spellDiff->SpellID[x]))
            {
                if (spellDiff->SpellID[x] > 0)//don't show error if spell is <= 0, not all modes have spells and there are unknown negative values
                    LOG_ERROR << fmt::format("spelldifficulty_dbc: spell %i at field id:%u at spellid%i does not exist in SpellStore (spell.dbc), loaded as 0", spellDiff->SpellID[x], spellDiff->ID, x);
    ...
    {
        LOG_ERROR << fmt::format("Incorrect DataDir value in worldserver.conf or ALL required *.dbc files (%d) not found by path: %sdbc", DBCFileCount, dataPath.c_str());
        exit(1);
    }
    else if (!bad_dbc_files.empty())
    {
		...
		LOG_ERROR << fmt::format("Some required *.dbc files (%u from %d) not found or not compatible:\n%s", (uint32)bad_dbc_files.size(), DBCFileCount, str.c_str());
        exit(1);
    }
    ...
    {
		LOG_ERROR << fmt::format("You have _outdated_ DBC files. Please extract correct versions from current using client.");
        exit(1);
    }

	std::cout << fmt::format(">> Initialized {} data stores in {} ms", DBCFileCount, GetMSTimeDiffToNow(oldMSTime)) << std::endl;
...
CharSectionsEntry const* GetCharSectionEntry(uint8 race, CharSectionType genType, uint8 gender, uint8 type, uint8 color)
{
    ...
    for (auto const& section : Containers::MapEqualRange(sCharSectionMap, key))
    {
...
{
   auto itr = sCharStartOutfitMap.find(race | (class_ << 8) | (gender << 16));
